...
fn main() {
    ...
    let (mut rules, messages) = load_file(&args[1]);

    let rule8 = vec![Rule::Set(vec![42]), Rule::Set(vec![42, 8])];
    let rule11 = vec![Rule::Set(vec![42, 31]), Rule::Set(vec![42, 11, 31])];
